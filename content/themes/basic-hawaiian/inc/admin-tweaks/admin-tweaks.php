...
function admin_style() {
    wp_enqueue_style( 'admin-style', get_stylesheet_directory_uri() . '/inc/admin-tweaks/admin-style.css' );
}
...
function toolbar_tweaks() {
	...
	$wp_admin_bar->remove_menu( 'nav_menu_item' );
...
function add_useful_toolbar_menu() {
	...
	if ( current_user_can('edit_posts') ) {
		...
		// Options
		// Roles: Employees & Above
		if ( get_current_user_role() === 'administrator' || get_current_user_role() === 'employee' || current_user_can('activate_plugins') ) :
			$wp_admin_bar->add_menu( array(
				'id' => 'options',
				'title' => 'Options',
				'meta' => array(),
